...
def medianaValor(sequence):
    sorted_seq = sorted(sequence)  # Ordenar a sequência
    length = len(sorted_seq)
    
    if length % 2 == 0:
        mid1 = organizar_seq[length // 2 - 1]
        mid2 = organizar_seq[length // 2]
...
def calcular_mediana(sequence):
    ...
    for num in sequence:
        ...
        median_value = median(current_sequence)
        medians.append(round(median_value, 1))
    
    return medians

sequence = eval(input())  # Receber a sequência como uma lista
