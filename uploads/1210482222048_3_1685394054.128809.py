...
Original file is located at
    https://colab.research.google.com/drive/1P4_lZXnkDHLFXVlnblp7YWE4UptaOlFp
...
for item in range(len(sequence)):
  ...
  for item2 in range(item+1):
    armazenar.append(sequence[item2])
    armazenar.sort()

  if (item+1)%2==0:
    media = round(len(armazenar)/2)
    mediana = (armazenar[media-1] + armazenar[media])/2
  else:
    media = round(len(armazenar)/2 - 0.5)
    mediana = armazenar[media]
