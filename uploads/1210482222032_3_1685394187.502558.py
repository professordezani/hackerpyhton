...
lista = eval(input()) # eval serve para transformar a string "[1, 2, 3]" numa lista [1, 2, 3]
...
for numero in range(len(lista)):
  ...
  for numero2 in range(numero+1):
    lista_ordenada.append(lista[numero2])
    lista_ordenada.sort()

  if (numero+1) %2 ==0:
    media = round(len(lista_ordenada)/2)
    mediana = (lista_ordenada[media-1] + lista_ordenada[media])/2

  else:
    media = round(len(lista_ordenada)/2 - 0.5)
    mediana = lista_ordenada[media]
  resultado.append(float(round(mediana, 1)))
